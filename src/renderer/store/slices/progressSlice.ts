...
import type { ProgressState, LessonProgressUpdate, CourseProgress, Course } from '@/types'
import { loadProgressData, saveProgressData } from '@/services/fileSystem'
import { calculateTotalLessons } from '@/services/courseLoader'
...
const progressSlice = createSlice({
  ...
  reducers: {
    ...
    initializeCourseProgressFromCourse: (state, action: PayloadAction<Course>) => {
      const course = action.payload
      const totalLessons = calculateTotalLessons(course)

      if (!state.courses[course.id]) {
        ...
        // Course already exists, just update totalLessons
        state.courses[course.id].totalLessons = totalLessons
      }
      // Do NOT set isDirty flag - this is initialization, not user action
    }
  },
